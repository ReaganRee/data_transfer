...
if __name__ == "__main__":
    ...
    #     "fasta"))
    counter = 0
    all_sequences_with_ind = []

    # pos_label_right = []
    #
    # pos_label_left = []
    for i in range(1000):
        pos_attention_scores_right_index = []
        pos_attention_scores_left_index = []
        print(i)
        ...
        for ind in pos_index:
            ...
            if int(ind) >= 20 and int(ind) < len(dict_out1[pos_records[i].id]) - 20:
                ...
                if pos_score > 0.004034567:#0.003214796:
                    pos_attention_scores_right_index.append(ind)
                elif pos_score <= 0.002185390:#0.003214796:
                    pos_attention_scores_left_index.append(ind)
                ...
                if pos_score > 0.4034567:
                    pos_attention_scores_right_index.append(ind)
                elif pos_score <= 0.2185390:
                    pos_attention_scores_left_index.append(ind)
        all_sequences_with_ind.append([pos_attention_scores_right_index, pos_attention_scores_left_index])
    import pickle

    with open('/Users/reagan/Desktop/3UTRBERT_visualiztion/conservation_score/index_for_conservation_all_small_range', 'wb') as fp:
        pickle.dump(all_sequences_with_ind, fp)
